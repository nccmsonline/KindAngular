...
export class OverTimePassingComponent implements OnInit {
  ...
  constructor(private http: HttpClient, @Inject('BASE_URL') private original_url : string, public dialog: MatDialog,) { 
  ...
  {
    this.isLoadingResults=true;
    this.http.get(this.original_url+"/hr/hr/getEmployeeListOT?serverip="+this.ServerIP+"&fyuser="+this.FYUSER+"&boid="+this.boid+"&empid="+ this.empid+"&dated="+this.TodayDay+"&IsConfirmed=Y").subscribe((res)=>{
    this.data=res;
    this.itemDisplay=res;
    this.itemDisplay=this.itemDisplay.Table;
    this.fieldArray.data = this.itemDisplay;
