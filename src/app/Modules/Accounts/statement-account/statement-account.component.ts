...
//  fieldArray: Array<any> = [];
  ...
  constructor(public dialog: MatDialog, private fb: FormBuilder,
    private modalService: NgbModal,public AppUser:Global,  private activatedRoute: ActivatedRoute,
    private http: HttpClient
  ) {
   // this.isLoadingResults=true;

   this.flag =AppUser.checknull( this.activatedRoute.snapshot.paramMap.get('flag'),'string');
    this.createForm();  
   ...
   if(this.flag=="O")
   {this.mindate=new Date("01-Dec-2021");
    if(this.mindate> new Date(this.fstartDate))
    {
      this.fstartDate=this.mindate;
    }
   }
   else
   {this.mindate=new Date(this.fstartDate);}   
...
this.userid = currentUser['USERID'];
  ...
  getAccountStatements(search)
  {
    ...
    // }));
    return this.http.get(this.original_url+"/Accounts/Accounts/getAccountList?token="+this.token+"&term="+search+"&flag="+this.flag).pipe(map((res : any[])=>{
      console.log("res",res);
      return res;
