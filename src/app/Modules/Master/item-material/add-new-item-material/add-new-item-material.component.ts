...
@Component({
  selector: 'app-add-new-item-material',
  templateUrl: './add-new-item-material.component.html',
  styleUrls: ['./add-new-item-material.component.css']
})
export class AddNewItemMaterialComponent implements OnInit {
  ...
  // expression="";
  // expression1="";
    constructor(public dialog: MatDialog,public dialogRef: MatDialogRef<AddNewItemMaterialComponent>,
      private http: HttpClient, @Inject(MAT_DIALOG_DATA) public data: any,private router: Router) { 
        ...
        if(parseInt(this.lCategoryId)>0)
        {
          ...
          this.title="Edit Item Material";
          // http.get(this.original_url+"/Master/getItemMaterialDetail?materialcatgid="+this.lCategoryId).subscribe((res)=>{
          //   this.allData=res;
    ...
    saveCategory()
    {
      ...
      if(this.validateDetail(this.newData))
      {
         ...
         // .set('parameter', JSON.stringify(detail))
          ...
          this.http.post(this.original_url+"/Master/SaveItemMaterial", params.toString(), {
