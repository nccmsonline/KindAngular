...
import { MachineListPopupComponent } from './machine-list-popup/machine-list-popup.component';
...
export class MachineMasterComponent implements OnInit {
  ...
  editmachine(data)
{
  var mcid=data.MACHINEID;
  this.router.navigate(['/add-machine-master/'+mcid+'/edit']);
}
...
refreshmachine()
{
  ...
  this.http.get(this.original_url+ "/Masters/MachineMaster/MachineMasterList?coid="+this.globalVar.CommpanyId+"&boid="+this.globalVar.BranchId+"&Pagenumber=1&Pagesize=100")
  .subscribe((data: any[]) => {
