...
export class AddNewDieComponent implements OnInit {
  isLoadingResults:boolean;ServerIP:any;FYUSER:any;boid:any;itemDisplay:any;routeID:any;routeAction:any;userid:any;token:any;
...
this.userid = currentUser['USERID'];
    ...
    http.get(this.original_url+"/Production/DieAndTools/loadDieFormData?token="+this.token).subscribe((res: any[])=> {
   ...
   {
     //this.original_url
      http.get(this.original_url+"/Production/DieAndTools/getDieData?dieid="+ this.routeID+"&token="+this.token).subscribe((res: any[])=> {
  ...
  savedata(data)
  {
    ...
    if(this.validatData(data))
    {
      ...
      mData.DIECODE=mData.DIECODE+' '+ frameDesc+'/'+poleDesc;
      if(data.ELESTANDARDID>0)
      {mData.DIECODE=mData.DIECODE+'-'+ electStandard}
